...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "𝐒𝐇𝐈𝐕𝐀𝐍𝐆-𝐗-𝐔𝐒𝐄𝐑𝐁𝐎𝐓 𝐏𝐋𝐔𝐆𝐈𝐍𝐒"
            ac.align = "l"
            for x in split_list(sorted(CMD_HELP.keys()), 2):
                ac.add_row([x[0], x[1] if len(x) >= 2 else None])
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• Mᴏᴅᴜʟᴇꜱ Pʀᴏᴠɪᴅᴇᴅ Bʏ 𝐒𝐇𝐈𝐕𝐀𝐍𝐆-𝐗-𝐔𝐒𝐄𝐑𝐁𝐎𝐓•",
    ...
    if help_arg:
        if help_arg in CMD_HELP:
            ...
            this_command += "Mᴏᴅᴜʟᴇꜱ Pʀᴏᴠɪᴅᴇᴅ Bʏ 𝐒𝐇𝐈𝐕𝐀𝐍𝐆-𝐗-𝐔𝐒𝐄𝐑𝐁𝐎𝐓 "
            await edit_or_reply(
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
            message, f"```{str(ac)}```\n• @Japanese_Userbot_Support × Mᴏᴅᴜʟᴇꜱ Pʀᴏᴠɪᴅᴇᴅ Bʏ 𝐒𝐇𝐈𝐕𝐀𝐍𝐆-𝐗-𝐔𝐒𝐄𝐑𝐁𝐎𝐓 •"
        )
        await message.reply(
